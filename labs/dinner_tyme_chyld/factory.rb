...
def create_plate
  plate = [] #local variable for plate
...
def put_plate_in_menus(plate)
  puts "List of menus (or new name): #{@menus.keys.join(', ')}?" #provide the user a list of all of the menus currently available OR type in a new one. 
  menu_name = gets.chomp.downcase
  puts "What day (sunday to saturday)?"
  menu_day = gets.chomp.downcase
  @menus[menu_name] = {} if @menus[menu_name].nil? #this is a one line if statement, which reads, "If @menus... is true, then do @menus[menu name]"
  @menus[menu_name][menu_day] = plate #this is a hash inside of a hash
  #--^---HASH---^--=^=KEY=^==
end
...
def add_menu_to_person
  ...
  @people[person_name] = create_person(person_name) if @people[person_name].nil? #@people is a hash that contains all of the people in it. If a person doesn't exist, go ahead and create a new person.
